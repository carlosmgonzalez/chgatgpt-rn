...
import { ReText, opacity } from "react-native-redash";
...
import { content } from "@/constants/ContentIntro";
...
export const AnimatedIntro = () => {
    ...
    (newWidth) => {
      currentX.value = withDelay(
        ...
        withTiming(
          ...
          (finished) => {
            if (finished) {
              (canGoToNext.value = true), (isAtStart.value = true);
            }
          }
        )
      );
    },
    [labelWidth, currentIndex, half]
  );
  ...
  return (
    <Animated.View style={[{ flex: 1 }, style1]}>
